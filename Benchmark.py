...
for index in testList:
    ...
    for graph in graphList:
        ...
        timer=time.time()
        subprocess.run(['python',"GenerateLP.py",graph,graph[:-5]+'lp'],stdout=subprocess.PIPE)
        generationTime=time.time()-timer
        print('Generated ILP:',count,'/',len(graphList),'\n','Took',generationTime)
        processes.append((fileList[index]+'/'+ os.path.basename(graph)[:-6],generationTime,subprocess.run(['glpsol','--lp'+flags,graph[:-5]+'lp'],stdout=subprocess.PIPE,universal_newlines=True)))
        #,'-o',(graph[:-5]+'solution') oben hinzuügen um die lösungen zu speuchern
        print('Finished solving of ilp:',count,'/',len(graphList))
        count=count+1

outputFile=open(resultFolder+resultFile,'w')
date=time.strftime('%c')
outputFile.write("Report:"+date+'\n')
...
for process in processes:
    ...
    print(processOut)
    solPosEnd=processOut.rfind('tree is empty')
    solPosStart=processOut.rfind('mip =',0,solPosEnd)
    print(solPosEnd)
    print(solPosStart)
    solString=processOut[solPosStart+len('mip ='):solPosEnd]
    timePosEnd=processOut.rfind('secs',0)
    timePosStart=processOut.rfind('Time used:',0,timePosEnd)
    timeStr=processOut[timePosStart+len('Time used:'):timePosEnd]
